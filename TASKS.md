...
- 2026-05-27: Added Content Factory MVP as a separate local-first queue, not a Guild Worker Team runtime: `scripts/content_factory.py`, `docs/content-factory/MVP.md`, and n8n workflow skeletons for candidate generation and approval. SQLite lives under `%LOCALAPPDATA%\hermes\content-factory`; artifacts live under ignored `_runtime/content-factory/`.
- 2026-05-27: Added ownership-first recovery docs for learning the codebase without expanding scope: `NEXT_SESSION_OWNERSHIP_HANDOFF.md` and `docs/learning/GUILD_MVP_OWNERSHIP_MAP.md`. Current teaching path is still `task -> blackboard -> worker -> result/status -> dashboard/CLI`.
- 2026-05-30: Stabilized Guild MVP around a visible, keyless E2E demo. Added `local-file-writer` as the default deterministic cartridge, extended worker output handling with scoped `file_outputs`, added temp-DB E2E and visible launcher scripts, introduced Hermes finalizer terminal flow, enriched dashboard repair/repaired-state display, and hardened provider adapters with retries/encoding fixes.
- 2026-05-31: Path/cache audit started. Current boundary: workspace root, `_runtime`, and `guild-workspaces` are real directories; `_obsidian_vault` is a junction to `D:\HermesVault\MemoryCore` and must not be treated as cache. Runtime state is split across `%LOCALAPPDATA%\hermes`, ignored workspace `_runtime`, ignored `guild-workspaces`, and `%TEMP%` demo SQLite files. Added `config/runtime-paths.json`, read-only `scripts/audit-runtime-paths.ps1`, and dry-run-only `scripts/cleanup-runtime-paths.ps1`. See `docs/maintenance/PATH_CACHE_AUDIT_2026-05-31.md`.
- 2026-05-31: Dry-run route smoke pass/fixes: path audit, cleanup reporter, local provider adapter, worker terminal `-DryRun`, dashboard `/api/wake` `dry_run=true` with temp DB, worker-agent no-claim path, Telegram dry-run, daily brief dry-run, Content Factory approval dry-run, and shared cognition dry-run. Fixed `hermes-healthcheck.ps1 -DryRun` so it does not require Hermes CLI or mutate blackboard; fixed timezone fallback in `content_factory.py` and `build-daily-brief.py`; fixed `content_factory.py send-approval --dry-run` so it no longer writes approval events/status.
- 2026-05-31: Added shared session-close memory route. `scripts/close-session-memory.ps1` produces a distilled dry-run candidate and, with `-Apply`, appends the daily note and rewrites `System/Assistant/Shared/Current State.md`. Added `config/session-memory.json` so the route is configurable but bounded by `allowed_writes` and `forbidden`; negative smoke confirmed `.env` is blocked. Added `skills/session-close-memory/SKILL.md` as the procedural wrapper so future agents know when to use the runtime route. Applied checkpoints to the vault after explicit approval for writing through `_obsidian_vault`.
- 2026-05-31: Hardened Hermes invocation infrastructure. Added `scripts/Resolve-HermesCli.ps1`, updated `hermes-healthcheck.ps1` to resolve the real Hermes CLI and support non-mutating `-CheckOnly`, added secretary-mode `scripts/invoke-hermes.ps1`, and updated `scripts/invoke-hermes-guild.ps1` to inject shared Current State plus support `-DryRun`. Read-only status showed Hermes CLI works, OpenRouter is configured, OpenAI Codex auth is logged in, scheduled jobs exist, and the Hermes gateway is currently stopped.

## Next Small Actions

1. Run `scripts/audit-runtime-paths.ps1` before any cleanup and review path roles/link targets.
2. Run `scripts/cleanup-runtime-paths.ps1 -MeasureSize` to inspect reclaim candidates; only add an apply/delete mode after manual review and explicit approval.
3. Use `scripts/close-session-memory.ps1 -DryRun` before applying session memory; apply only distilled summaries, not raw logs.
4. Make real provider-backed workers write scoped files reliably, using the `local-file-writer` smoke as the contract baseline.
5. Move final assembly from API-owned `hermes_finalize_v1` into a normal worker-claimed `final_review` or stronger `join_review` path after scoped file writes are stable.
6. Keep using ignored `_runtime/provider-secrets.local.ps1` for provider keys; do not expose or commit key values.
7. Start/check Hermes gateway only with explicit approval; current scheduled jobs will not fire while the gateway is stopped.
